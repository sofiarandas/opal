...
object TACtoBC {
  ...
  def translateSingleTACtoBC(tac: AITACode[TACMethodParameter, ValueInformation]): Array[Instruction] = {
    val byteCodeArray = new ArrayBuffer[Instruction]()
    tac.stmts.foreach {
      case Assignment(_, _, StringConst(_, value)) =>
        byteCodeArray += LoadString(value)
      case Assignment(_, _, GetStatic(_, declaringClass, name, declaredFieldType)) =>
        byteCodeArray += GETSTATIC(declaringClass, name, declaredFieldType)
      case VirtualMethodCall(_, declaringClass, isInterface, name, descriptor, _, _) =>
        if(isInterface) {
          ...
          byteCodeArray += INVOKEVIRTUAL(declaringClass, name, descriptor)
        }
      case Return(_) =>
        byteCodeArray += RETURN
      case _ =>
    }

    byteCodeArray.toArray
  }
